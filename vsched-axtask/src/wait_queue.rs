...
pub struct WaitQueue {
    pub(crate) queue: SpinNoIrq<VecDeque<BaseTaskRef>>,
}

pub(crate) type WaitQueueGuard<'a> = SpinNoIrqGuard<'a, VecDeque<BaseTaskRef>>;
...
impl WaitQueue {
    ...
    pub fn wait(&self) {
        ...
        current_guard::<NoPreemptIrqSave>().blocked_resched(wq);
...
fn unblock_one_task(task: BaseTaskRef, resched: bool) {
    ...
    log::debug!("unblock task {:?}", task.task_ext().id_name(),);
    ...
    current_guard::<NoPreemptIrqSave>().unblock_task(task, resched);
}
