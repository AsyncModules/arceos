...
use crate::task::Task;
use crate::wait_queue::{WaitQueue, WaitQueueGuard};
use axhal::percpu::this_cpu_id;
use base_task::{BaseTaskRef, TaskExtRef, TaskState};
...
percpu_static! {
    EXITED_TASKS: VecDeque<BaseTaskRef> = VecDeque::new(),
...
impl<G: BaseGuard> CurrentGuard<G> {
    ...
    pub fn add_task(&mut self, task: BaseTaskRef) {
        debug!(
            "task add: {} on run_queue {}",
            task.task_ext().id_name(),
            this_cpu_id()
        );
    ...
    #[inline(always)]
    pub fn unblock_task(&mut self, task: BaseTaskRef, resched: bool) {
        let task_id_name = task.task_ext().id_name();
...
impl<G: BaseGuard> CurrentGuard<G> {
    ...
    pub fn yield_current(&mut self) {
        let cpu_id = this_cpu_id();
        let curr = vsched_apis::current(cpu_id);
        trace!("task yield: {}", curr.task_ext().id_name());
        assert!(curr.is_running());
        vsched_apis::yield_now(cpu_id);
    }

    #[cfg(feature = "smp")]
    fn migrate_entry(&mut self, migrated_task: BaseTaskRef) {
    ...
    #[cfg(feature = "smp")]
    pub fn migrate_current(&mut self, migration_task: BaseTaskRef) {
        let cpu_id = this_cpu_id();
        let curr = vsched_apis::current(cpu_id);
        trace!("task migrate: {}", curr.task_ext().id_name());
        assert!(curr.is_running());
    ...
    pub fn blocked_resched(&mut self, mut wq_guard: WaitQueueGuard) {
        ...
        debug!("task block: {}", curr.task_ext().id_name());
        vsched_apis::resched(cpu_id);
    }
...
#[cfg(feature = "smp")]
pub(crate) fn migrate_entry(migrated_task: BaseTaskRef) {
