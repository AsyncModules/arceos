...
/// initialized.
pub fn current_may_uninit() -> Option<BaseTaskRef> {
...
pub fn spawn_task(task_ref: BaseTaskRef) -> BaseTaskRef {
    current_guard::<NoPreemptIrqSave>().add_task(task_ref.clone());
    task_ref
}
...
pub fn spawn_raw<F>(f: F, name: String, stack_size: usize) -> BaseTaskRef
where
...
///
/// Returns the task reference.
pub fn spawn_raw_f<F>(f: F, name: String) -> BaseTaskRef
where
...
///
/// Returns the task reference.
pub fn spawn<F>(f: F) -> BaseTaskRef
where
    F: FnOnce() + Send + 'static,
{
...
///
/// Returns the task reference.
pub fn spawn_f<F>(f: F) -> BaseTaskRef
where
    F: Future<Output = ()> + Send + 'static,
{
    spawn_raw_f(f, "".into())
